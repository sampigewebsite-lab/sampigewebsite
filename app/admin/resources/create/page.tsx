...
export default function CreateResource() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Upload file to the "documents" storage bucket in Supabase
      ...
      if (uploadError) {
        if (uploadError.message.includes('bucket not found')) {
          toast.error('Storage bucket not found. Please create "documents" bucket in Supabase.')
        } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase.from('resources').insert([dataToInsert])
      ...
      <main className="flex-1 p-4 md:p-8 overflow-y-auto">
        <div className="max-w-4xl mx-auto">
          <div className="flex items-center gap-4 mb-8">
            ...
            <div>
              <h1 className="text-2xl md:text-3xl font-bold text-white">Add Resource</h1>
              <p className="text-gray-400 mt-1 text-sm">Register a new downloadable document</p>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="bg-[#1A1A1A] rounded-xl p-4 md:p-6 border border-gold-500/10 space-y-4 md:space-y-6">
                    ...
                    <label className="w-full h-12 border border-dashed border-gray-700 rounded-lg flex items-center justify-center bg-black/50 cursor-pointer hover:border-gold-500/30 transition-colors gap-2">
                      {uploading ? (
                        ...
                        <>
                          ...
                          <span className="text-xs text-gray-500">Choose file (PDF, DOCX, ZIP • Max 10MB)</span>
                        </>
                      )}
                      <input
                        type="file"
              ...
              <div>
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  />
                  <span className="text-gray-300 text-sm">Publish on Frontend Directory</span>
                </label>
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-4">
              ...
              >
                ...
                {loading ? 'Creating...' : 'Register Resource'}
              </button>
              <button
                type="button"
                onClick={() => router.push('/admin/resources')}
