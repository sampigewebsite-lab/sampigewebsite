...
export default function EditGalleryAlbum() {
  ...
  const fetchAlbum = async () => {
    ...
    try {
      ...
      const { data: galleryImages, error: imageError } = await supabase
        .from('gallery_images')
        .select('*')
        .eq('album_id', id)
      ...
      <main className="flex-1 p-4 md:p-8 overflow-y-auto">
        <div className="max-w-4xl mx-auto">
          <div className="flex items-center gap-4 mb-8">
            ...
            <div>
              <h1 className="text-2xl md:text-3xl font-bold text-white">Edit Album</h1>
              <p className="text-gray-400 mt-1 text-sm md:text-base">Update album details</p>
            </div>
          </div>
